...
- Key behaviors:
  ...
  - `PersistentPreRunE` initializes runtime state for normal commands, but skips `doctor` so environment diagnostics can inspect config read-only without creating missing files.
...
- All subcommand files (`record.go`, `replay.go`, `diff.go`, `report.go`, `session.go`, `doctor.go`, `version.go`) depend on `rootCmd` to register themselves via `rootCmd.AddCommand()`.
...
- The `cfgFile` variable is used by runtime initialization and by `doctor`'s read-only config inspection path.
